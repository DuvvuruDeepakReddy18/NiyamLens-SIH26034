...
export default function SystemTrust({ workspace, online, install, historyCount, lastSyncAt, onNavigate }) {
  ...
  const runtime = [
    ...
    { icon: managed ? Cloud : Database, label: 'Workspace mode', value: managed ? 'Managed cloud workspace' : 'Local-only workspace', detail: managed ? 'Sealed evidence is queued locally, then uploaded to private storage and server-verified.' : 'No managed evidence upload is attempted.' },
