...
export default async function handler(req, res) {
  try {
    ...
    if (!uuid(operationId) || !Number.isInteger(baseVersion) || baseVersion < 1 || !DISPOSITIONS.includes(status) || typeof reason !== 'string' || reason.trim().length < 12 || reason.length > 4000) throw new HttpError(400, 'A versioned review with a clear reason is required.')
